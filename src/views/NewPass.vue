    ...
    <div class="limiter">
      <div class="container-login100">
        <div class="wrap-login100 p-l-55 p-r-55 p-t-65 p-b-50">
          <form class="login100-form validate-form" v-on:submit.prevent="resetp">
            ...
            <div v-if="rp">
              ...
              <router-link class="txt2 hov1" to="/" exact>главную</router-link>
              </div>
            ...
            <div class="wrap-input100 rs1 validate-input" v-bind:class="{ red: passError }" data-validate="Password is required">
              <input class="input100" type="password" name="pass2" v-model="pass2" required>
              <span class="focus-input100-1"></span>
              <span class="focus-input100-2"></span>
            </div>
  ...
  methods: {
     async resetp() {
        ...
        if (this.pass===this.pass2) {
            await fetch('http://localhost:5000/auth/users/reset_password_confirm/', {
            method: 'POST',
            headers: {
               'Content-Type': 'application/json'
            },
            body: JSON.stringify({uid: this.uid, token: this.token, new_password: this.pass})
            })
            .then((res)=>{
               if (res.status==204) {
                  return {"ok": true}
                ...
                if (data.ok) {
                  ...
                  if (data.new_password) {
                    for (let i=0; i<data.new_password.length; i++)
                      this.detail += data.new_password[i]
                  }
                  else
                    this.detail = 'Ошибка. Повторите действие позже'
                }
                console.log(data)
            });
        }
          ...
          this.passError = true;
          this.detail = 'Пароли не совпадают';
        }
