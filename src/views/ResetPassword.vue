      ...
      <div class="container-login100">
        <div class="wrap-login100 p-l-55 p-r-55 p-t-65 p-b-50">
          <form class="login100-form validate-form" v-on:submit.prevent="resetp">
            ...
            <span class="gray">
              Вам на почту будет отправлена ссылка для сброса пароля
            </span>
            ...
            <p v-show="showSend" class="loading">Отправление</p>

            <div class="text-center p-t-45">
              <span class="txt1">
                Передумали?
              </span>
              <router-link class="txt2 hov1" to="/auth" exact>Назад</router-link>
            </div>
            </div> 
          </form>
        </div>
...
export default {
  ...
  methods: {
     async resetp() {
        ...
        await fetch('http://localhost:5000/auth/users/reset_password/', {
            method: 'POST',
            headers: {
        ...
        .then(data=> {
            ...
            if (data.ok) {
              ...
              if (data.email)
              this.detail = 'Не существует такого email адреса'
              else this.detail = 'Пользователь не найден'
            }
          console.log(data)
        });
